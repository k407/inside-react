...
function Item({ item }) {
  return (
    <article className={'item' + (item.favorite ? ' item_favorite' : '')}>
      ...
      <Favorite active={item.favorite} id={item.id} />
    </article>
  );
}
...
function Favorite({ active, id }) {
  return active ? (
    <button type="button" className="unfavorite" onClick={() => unfavorite(id)}>
      <img className="favorite_icon" src="images/heart-sharp.png" alt="" /> Unfavorite
    </button>
  ) : (
    <button type="button" className="favorite" onClick={() => favorite(id)}>
